...
- **Rune of Return (`src/engine/magic/runeOfReturn.ts`):** a limited-charge item (`RuneOfReturnItem`, max 3 charges) that channels a multi-turn teleport-to-town spell, replacing the removed campaign-specific town-return fixtures (P-03) with a fixed, pack-neutral mechanic. `ChannelRuneOfReturnAction` is the explicit "keep channeling" action a player repeats each turn (via the `channel_rune_of_return` command) — any *other* turn-consuming action (including movement, unless the Unbound Casting track is maxed) breaks it, per the shared `GameCommand`/`EngineCommandBus` decoupling pattern rather than a bespoke UI hook.
  ...
  - **Implementation:** modeled as a status effect (`RUNE_OF_RETURN_STATUS`), consistent with the existing dormant-actor/status-tick scheduling (poison, slow, etc. tick the same way) rather than a bespoke per-turn loop. `StatusEffect` gained an optional `data` scalar bag (mirroring `GameEventBase.data`) so a handler can carry small per-instance state — here, the last-observed HP and whether this turn's action continued the channel — without a bespoke shape per effect. One documented timing tradeoff: `player-status-tick` runs *before* monster turns within a single player-turn cycle, so a monster counter-attack's damage is observed on the *next* tick rather than instantly.
  - **Pack-neutrality:** the mechanic's rules are engine-fixed; only presentation varies. `GameContentManifest.runeOfReturn` (optional) supplies `attunementNpcId` — interacting with that NPC (`GameEngine.interactWithNpc`, mirroring the existing `quest.victoryNpcId` pattern) fully refills charges, free and instant — and `trackNames` for pack-specific display strings. The base pack (`cotw`) points this at a new town NPC, "Thrain the Rune-Smith," stationed at Gunther's forge; a manifest with no `runeOfReturn` config simply has no in-town refill trigger (`warcraft` builds unaffected). A visible channel-progress UI element is intentionally not built yet — flagged as a follow-up UI pass.
...
- **Focus & Modal Isolation:**
  ...
  - **Registered:** every in-game modal. Inventory, targeting, spellbook, diagnostics, level-up, pacts (both the keyboard path and click), context help, compendium, the radial menu, the shop overlay, choice, save & quit, settings/keybinds, and save-code. (Dwarven Winch and town-return modals were removed 2026-09-16 along with their underlying mechanics — see P-03.)
...
- **2026-09-16: the Rune of Return was implemented as their pack-neutral replacement** (§3's Content Extensibility Model has the full writeup) — a fixed engine mechanic (channel timing, banking, mobility, interrupt rules) with only presentation and the town refill trigger left to content. Genuinely new engine-owned capability, not extracted campaign content, so it doesn't reduce the remaining item below.
