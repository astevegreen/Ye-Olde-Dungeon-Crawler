...
- Hard Constraints:
  ...
  - Never modify `src/engine/actions/actionPipeline.ts`, `src/engine/engine.ts`, or `src/engine/storage/migrator.ts` except under ARCHITECTURE.md §8.1 (confirmed bug fix, additive migration step, or an explicitly requested Planned Work item) — that exception is rare for this persona.
