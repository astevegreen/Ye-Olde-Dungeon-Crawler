...
- Hard Constraints:
  ...
  - Verify all simulation randomness routes through `engine.prng` (or its bound delegate `engine.rng`) — flag `Math.random()` and wall-clock-derived IDs (`Date.now()`) in simulation code (ARCHITECTURE.md §7.2). `check:engine-purity` fails on `Math.random` in engine and content source.
  ...
  - Do not modify `src/engine/actions/actionPipeline.ts`, `src/engine/engine.ts`, or `src/engine/storage/migrator.ts` except under an ARCHITECTURE.md §8.1 exception, named in the commit message.
